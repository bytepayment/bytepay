...
async function onDevRepoChanged() {
  ...
  devTasksForTabel.value = devTasks.value.filter(
    (item) => item.repo_id === selectedRepo.repo_id
  );
}
        ...
        <el-tabs class="tabs" v-model="activeTabIndex" :stretch="true" type="border-card">
          ...
          <el-tab-pane label="My Repositories" class="repo-list">
            ...
            >
              ...
              >
                <div>
                  {{ item?.meta?.description || "Your repo have no description 😳 " }}
                </div>
              </el-collapse-item>
            </el-collapse>
          </el-tab-pane>
          ...
          <el-tab-pane label="My Task" class="repo-list">
            <el-collapse
              v-model="activeDevRepoIndex"
              accordion
              @change="onDevRepoChanged()"
            >
              <el-collapse-item
            ...
            <el-table-column prop="task_url" label="Link">
              <template #default="scope">
                <el-icon
                  size="25"
                  color="#6667AB"
                  @click="gotoTaskUrl(scope.row.task_url)"
                >
                  <Link />
                </el-icon>
              </template>
