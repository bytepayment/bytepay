...
class BytePayService {
    ...
    public static async check(repoId: number, headers: Headers, body: any) {
        const hook_id = parseInt(headers["x-github-hook-id"]);
        const repos = await Repository.selectByRepoIdAndHookId(repoId, hook_id);
        if (!repos) {
            console.log(repoId, hook_id, "仓库id和hookid查不到结果");
            return false;
        }
        const result = createHmac("sha256", repos.webhookSecret).update(JSON.stringify(body)).digest("hex");
...
class RobotService {
    ...
    public static async taskPay(issueLoc: IssueLoc, params: { login: string; devLogin: string; hash: string; account: Blockchain }) {
        const url = params.account === Blockchain.NEAR ? Config.nearExplorerUrl + params.hash : `${Config.SubscanBaseUrl}/extrinsic/${params.hash || ""}`;
        const s = `
  \r\n**@${params.devLogin}:**\r\n
...
class UserRepository {
    ...
    private static async selectOne(where: Partial<User>): Promise<User> {
        const res = await this.collection.where(where).getOne<User>();
        if (res.ok) {
            return res.data;
        }
        throw new Error(res.error);
...
class Repository {
    public static async selectByRepoIdAndHookId(repoId: number, hookId: number) {
        const resp = await DB.collection(COLLECTION.repository).where({ repo_id: repoId, hook_id: hookId }).getOne();
        if (resp.ok) {
            return resp.data;
        }
