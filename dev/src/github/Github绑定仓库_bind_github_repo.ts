...
exports.main = async function (ctx: FunctionContext) {
    ...
    try {
        ...
        console.log(data, "这里是数据");
        const { id: hook_id, webhookSecret } = data;
        const dp = await DB.collection(TABLE_NAME_REPOS)
            .where({ owner_id, repo_id })
            .update({ owner_id, repo_id, owner_name, repo_name, meta, hook_id, webhookSecret }, { upsert: true });
        console.log(dp, "插入成功了吗");
...
async function existed(ownerId: string, repoId: string): Promise<boolean> {
    const res = await DB.collection(TABLE_NAME_REPOS).where({ owner_id: ownerId, repo_id: repoId }).count();
    if (!res.ok) {
